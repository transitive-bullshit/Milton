 ...
 * 29 Mar 2008
 * 
 * @note This is a modified version of tinyjson for use by Milton.  It includes 
 *    modifications to be a bit more leniant with extra commas after brackets 
 *    and to allow C-style comments.
 */
...
namespace json
{
	...
	public:
		...
		typedef	std::multimap< std::basic_string< Char >, variant >	object;	//    an object with json pairs
	...
	public:
		...
		class definition
		{
			...
			boost::spirit::rule< SCANNER > m_comment;
		...
		public:
			...
			definition(grammar const & self)
			{
				...
				m_object
					= ch_p('{') [ begin_object(self.m_stack) ] >>
					  //!(m_pair >> *(ch_p(',') >> m_pair)) >>  // original
               // tfischer: modified 11/29/2008 to allow flexible trailing commas and comments
                 *m_comment >> 
					  !(m_pair >> ((*(*m_comment >> ch_p(',') >> m_pair)) || (*m_comment >> ch_p(',') >> *(m_pair >> ch_p(',') >> *m_comment)))) >>
                 *m_comment >> 
               ...
               // tfischer: modified 11/29/2008 to allow flexible trailing commas and comments
                 *m_comment >> 
                 !(m_value >> ((*(*m_comment >> ch_p(',') >> m_value)) || (*m_comment >> ch_p(',') >> *(m_value >> ch_p(',') >> *m_comment)))) >>
                 *m_comment >> 
            ...
            m_comment 
               ...
               ;
#endif
            // TODO: (tfischer), figure out why C++ comments aren't recognized at all!!
            m_comment
               = (comment_p("/*", "*/")  // C   comments
               |  comment_p("//"))       // C++ comments
               ;
	...
	typename json::grammar< typename Iterator::value_type >::variant parse(Iterator const & szFirst, Iterator const & szEnd)
	{
      ...
      // tfischer: modified 11/29/2008; added 'typename' at beginning of line to get rid of g++ error
		...
		{
         // tfischer: modified 11/29/2008; added 'typename' after 'return' to get rid of g++ error

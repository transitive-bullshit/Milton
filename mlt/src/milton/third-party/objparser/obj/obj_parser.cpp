#include <third-party/objparser/obj/obj_parser.hpp>
...
// disable warnings for:  unary minus operator applied to unsigned type, result still unsigned
#pragma warning(disable : 4146)

/*#define return(x) \
   std::cerr << "return: " << __LINE__ << std::endl; \
   return((x));*/
...
bool obj::obj_parser::parse(std::istream& istream)
{
   ...
   bool retVal = (istream.fail() && istream.eof() && !istream.bad());
   
   if (!retVal && error_callback_) {
      error_callback_(line_number, "unknown parse error with input stream");
   }

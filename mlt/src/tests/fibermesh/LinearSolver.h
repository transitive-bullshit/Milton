...
class LinearSolver {
   public:
      inline LinearSolver(SparseMatrix &A)
         : m_A(A), m_dirty(true)
      {
         ASSERT(A.getN() > 0 && A.isSquare());
      }
      
      LinearSolver()
         : m_A(LinearSolver::s_dummyA), m_dirty(true)
      { }
       ...
       */
      virtual void setA(SparseMatrix &A);
